...
export async function generateMetadata({
  ...
  return {
    title: post.title,
    description: post.description || `Read about ${post.title} on K-TOOL blog`,
    openGraph: {
      title: post.title,
          ...
          {post.body &&
            post.body.map((part: ImageBlock | TextBlock) => {
              ...
              if (isBlock) {
                ...
                if (largerHeading) {
                  return (
                    <h3 key={part._key} className="text-2xl font-bold mt-9">
                      {text}
                    </h3>
                  );
                }
                ...
                if (smallerHeading) {
                  return (
                    <h4 key={part._key} className="text-xl font-bold mt-9">
                      {text}
                    </h4>
                  );
                }
